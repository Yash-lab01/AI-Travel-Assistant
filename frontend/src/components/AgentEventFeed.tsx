    ...
    <div className="agent-feed">
      {events.map((ev, i) => (
        <div key={i} className="agent-event">
          ...
          <span style={{ fontSize: 11 }}>{EVENT_ICONS[ev.event_type]}</span>
          <span>
            {ev.agent && <span style={{ color: 'var(--accent-gold)', fontWeight: 600 }}>{ev.agent}</span>}
            {ev.tool && <span style={{ color: 'var(--accent-teal)' }}> [{ev.tool}]</span>}
            {' '}{ev.message}
          </span>
        </div>
      ))}
