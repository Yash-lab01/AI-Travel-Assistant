...
export default function MapView({ stops, activeDay }: Props) {
  ...
  const updateLeafletMarkers = () => {
    ...
    stops.forEach((stop, index) => {
      ...
      const customIcon = L.divIcon({
        ...
        html: `
          <div style="
            ...
            cursor:pointer;">
            <span style="transform:rotate(45deg);color:#040e1f;
              font-family:'Sora',sans-serif;font-size:11px;font-weight:700;line-height:1;">
              ${index + 1}
            </span>
          </div>`,
        iconSize: [30, 30],
      ...
      const popupContent = `
        <div style="background:#0d192c;border:1px solid rgba(255,255,255,0.15);
          border-radius:10px;padding:10px 12px;color:#d8e3fb;
          font-family:'Outfit',sans-serif;min-width:200px;max-width:240px;">
          ${photoImg}
          ${stop.is_niche
            ? '<div style="color:#00DBE7;font-size:10px;font-weight:700;letter-spacing:.08em;margin-bottom:4px">💎 HIDDEN GEM</div>'
            : ''}
          <div style="font-family:'Playfair Display',serif;font-size:15px;font-weight:600;color:#fff;margin-bottom:4px">
            ${stop.name || 'Stop'}
          </div>
          <div style="font-size:12px;color:#909096;text-transform:capitalize;margin-bottom:6px">
            ${stop.category || 'Attraction'} · ${stop.duration_minutes || 60} min
          </div>
          ${stop.estimated_cost_usd !== undefined
            ? `<div style="font-size:12px;color:#FFBF00;font-weight:600">
                 ${formatCost(stop.estimated_cost_usd, stop.address || stop.description || stop.name)}
               </div>`
            : ''}
        </div>`;
      ...
      try {
        const marker = L.marker([lat, lon], { icon: customIcon })
          .bindPopup(popupContent, { className: 'leaflet-dark-popup' })
          .addTo(map);
    ...
    // Always defer polyline drawing with rAF + setTimeout so the map container
    // has settled its pixel origin — prevents the '_clipPoints x undefined' crash.
    if (latLngs.length >= 1) {
      requestAnimationFrame(() => {
        setTimeout(() => {
          try {
            ...
            // Silently suppress any residual Leaflet layout race conditions
          }
        }, 80);
      });
    }
  ...
  // Only re-render markers when stops/day change AND the map is already ready.
  // mapReadyRef prevents a duplicate render race on initial mount.
  useEffect(() => {
